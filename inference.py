...
class BLEUEvaluator:
    ...
    def evaluate(self, dataset_path, output_path="translations_comparison.txt"):
        ...
        for batch in progress_bar:
            ...
            with torch.no_grad():
                outputs = self.model.greedy_decode(
                    ...
                    max_length=self.max_length * 2  # ensure enough decode length
                )
        ...
        bleu_score = corpus_bleu(hypotheses, references, tokenize='zh')
        print(
            f"\nEvaluation Complete\n"
...
def main():
    ...
    # model = torch.load("model/checkpoints/model_003.pt", map_location=device)
    model = torch.load("model/checkpoints/latest_model.pt", map_location=device)
